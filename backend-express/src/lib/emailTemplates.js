...
const {
  BRAND, buildResponsiveEmail, emailButton, emailInfoCard, emailHighlightCard,
} = require('./emailLayout');
...
function interviewScheduled({ name, jobTitle, deadline }) {
  return {
    ...
    html: wrapHtml('Interview Invitation', `
      ...
      <p style="font-size: 18px; font-weight: bold; color: ${BRAND.aqua};">${deadline}</p>
...
function interviewPassedHrNotice({
  ...
  const verdictColor = interviewScore >= 70 ? BRAND.aqua : interviewScore >= 55 ? BRAND.warn : '#dc2626';
  return {
...
function finalSelected({ name, jobTitle, salary, startDate, message, employmentType, leavePolicy }) {
  ...
  return {
    ...
    html: wrapHtml(`Congratulations, ${name}!`, `
      ...
      <p><strong>Employment type:</strong> ${employmentType === 'internship' ? 'Internship' : 'Full-time'}</p>
      ${leavePolicy ? emailInfoCard(`<p style="margin:0"><strong>Leave policy:</strong> ${leavePolicy}</p>`) : ''}
      ...
      ${compBlock}
      ${message ? `<div style="margin:16px 0;color:${BRAND.body};line-height:1.6">${message}</div>` : ''}
      <p>Please accept or decline in the NeuroHR portal within <strong>5 business days</strong>, or reply to this email with any questions.</p>
      ${btn(`${config.appUrl}/dashboard/job-openings`, 'Review & Respond to Offer')}
  ...
  return {
    ...
    html: wrapHtml(`Payslip — ${monthLabel || month}`, `
      ...
      <table class="email-stack" role="presentation" style="width:100%;max-width:100%;border-collapse:collapse;font-size:14px;margin:16px 0">
        <tr style="background:${BRAND.aquaBg}"><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};color:${BRAND.heading}"><strong>Earnings</strong></td><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};text-align:right"></td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder}">Basic salary</td><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};text-align:right">${fmt(basic)}</td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder}">Allowance</td><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};text-align:right">${fmt(allowance)}</td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder}">Bonus</td><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};text-align:right">${fmt(bonus)}</td></tr>
        <tr style="background:${BRAND.aquaBg}"><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};color:${BRAND.heading}"><strong>Deductions</strong></td><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};text-align:right"></td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder}">Tax (TDS estimate)</td><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};text-align:right">${fmt(tax)}</td></tr>
        ${leaveDeduction ? `<tr><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder}">Leave deduction</td><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};text-align:right">${fmt(leaveDeduction)}</td></tr>` : ''}
        <tr><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder}">Other deductions</td><td style="padding:8px;border-bottom:1px solid ${BRAND.creamBorder};text-align:right">${fmt(Math.max(0, (deductions || 0) - (leaveDeduction || 0)))}</td></tr>
        <tr style="background:${BRAND.successBg}"><td style="padding:12px;font-size:16px;color:${BRAND.heading}"><strong>Net pay</strong></td><td style="padding:12px;text-align:right;font-size:18px;font-weight:bold;color:${BRAND.aqua}">${fmt(netPay)}</td></tr>
      </table>
      ${leaveSummaryHtml || ''}
...
function leaveRequestHrNotice({
  ...
  const balanceBlock = balanceSummary
    ? `<p style="font-size:13px;color:${BRAND.body};background:${BRAND.aquaBg};padding:12px;border-radius:8px;margin:16px 0"><strong>Leave balances:</strong> ${balanceSummary}</p>`
    : '';
  const exceedBlock = exceedsBalance
    ? `<p style="font-size:13px;color:${BRAND.warn};background:${BRAND.warnBg};padding:10px;border-radius:6px;margin:12px 0;border-left:4px solid ${BRAND.aqua}"><strong>Note:</strong> This request exceeds remaining balance — excess days may be deducted from payroll.</p>`
    : '';
  const reasonBlock = reason
    ? `<div style="margin:16px 0;padding:12px;background:${BRAND.cream};border-radius:8px;border-left:4px solid ${BRAND.aqua};color:${BRAND.body};line-height:1.6">${reason}</div>`
    : `<p style="color:${BRAND.muted}">No reason provided.</p>`;
